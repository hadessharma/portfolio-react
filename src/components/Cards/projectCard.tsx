    ...
    >
      ...
      <div className="flex flex-wrap gap-1 sm:gap-2 justify-start overflow-hidden">
        {project.stack.slice(0, 3).map((tech) => (
          <span
            key={tech}
            className="bg-paper-layer text-paper-ink text-xs font-medium px-2 sm:px-3 py-1 rounded-full border border-paper-edge whitespace-nowrap flex-shrink-0"
          >
            {tech}
          </span>
        ))}
        {project.stack.length > 3 && (
          <span className="bg-paper-layer text-paper-ink text-xs font-medium px-2 sm:px-3 py-1 rounded-full border border-paper-edge whitespace-nowrap flex-shrink-0">
            +{project.stack.length - 3}
          </span>
        )}
      </div>
